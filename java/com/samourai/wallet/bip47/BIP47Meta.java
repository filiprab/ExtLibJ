...
//    public static final int OUTGOING_LOOKAHEAD = 3;
    ...
    */
    ...
    public synchronized String[] getIncomingLookAhead(BIP47Wallet wallet)  {
...
//            info("APIFactory", "idx:" + idx + " , " + pcode);
            ...
            for(int i = idx; i < (idx + INCOMING_LOOKAHEAD); i++)   {
                try {
                    ...
                    BIP47Meta.getInstance().getIdx4AddrLookup().put(Bip47UtilJava.getInstance().getReceivePubKey(wallet, new PaymentCode(pcode), i, wallet.getParams()), i);
                    BIP47Meta.getInstance().getPCode4AddrLookup().put(Bip47UtilJava.getInstance().getReceivePubKey(wallet, new PaymentCode(pcode), i, wallet.getParams()), pcode.toString());
                    addrs.add(Bip47UtilJava.getInstance().getReceivePubKey(wallet, new PaymentCode(pcode), i, wallet.getParams()));
                }
                catch(Exception e) {
                    ;
                }
...
//            }
    ...
    public ArrayList<String> getUnspentAddresses(BIP47Wallet wallet, String pcode)    {
        ...
        try {
            ...
            if(idxs != null)    {
                for(int i = 0; i < idxs.size(); i++)   {
                    //info("BIP47Meta", "address has unspents:" + Bip47UtilJava.getInstance(ctx).getReceivePubKey(new PaymentCode(pcode), idxs.get(i)));
                    ret.add(Bip47UtilJava.getInstance().getReceivePubKey(wallet, new PaymentCode(pcode), idxs.get(i), wallet.getParams()));
                }
            }
