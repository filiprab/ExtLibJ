...
public class DLEQProof {
    ...
    public DLEQProof(final ECPoint g1, final ECPoint g2, final ECCurve curve, final BigInteger x) {
        this.x = x;
        ...
        this.h1 = curve.getMultiplier().multiply(g1, x);
        this.h2 = curve.getMultiplier().multiply(g2, x);
    ...
    public static void main(String args[])
    {
        ...
        final ECPoint G2 = ECKey.CURVE.getG();

        final BigInteger x = new ECKey().getPrivKey();
        final DLEQProof dleq = new DLEQProof(G1, G2, curve, x);
        ...
        final BigInteger c = new ECKey().getPrivKey();
        final BigInteger s = dleq.getS(c);

        // Check first point
        final ECPoint G1s = curve.getMultiplier().multiply(G1, s);
        final ECPoint H1c = curve.getMultiplier().multiply(H1, c);
        ...
        final ECPoint H2c = curve.getMultiplier().multiply(H2, c);
